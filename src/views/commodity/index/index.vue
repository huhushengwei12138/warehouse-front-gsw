<template>
  ...
  <div>
    <el-form class="searchForm">
      <el-form-item>
        <el-select v-model="params.storeId" style="width: 120px;" clearable placeholder="请选择仓库">
      ...
      <el-form-item style="margin-left: 10px;">
        <el-autocomplete
          v-model="params.supplyName"
          :fetch-suggestions="supplySearch"
          value-key="supplyName"
          clearable
          placeholder="供应商"
          style="width: 120px;"
        />
  ...
  <el-table ref="multipleTableRef" :data="commodityPageList" @selection-change="handleSelectionChange" style="width: 100%; margin-top: 10px;" table-layout="auto" size="large" border stripe>
    ...
    <el-table-column label="商品图片">
      <template #default="props">
          <el-image style="width: 60px; height: 60px" :src="props.row.imgs" fit="fill" />
      </template>
    </el-table-column>
...
// 获取所有仓库
const getStoreList = () => {
  get("/store/store-list").then(result => {
...
// 获取所有产地
const getPlaceList = () => {
  get("/place/place-list").then(result => {
...
// 获取所有单位
const getUnitList = () => {
  get("/unit/unit-list").then(result => {
...
// 供应商模糊查询框的联想输入
const supplySearch = (queryString, callback) => {
  const results = queryString
    ? supplyList.value.filter(createSupplyFilter(queryString))
    : supplyList.value
  // 回调方法，返回联想的数据
  callback(results);
}

const createSupplyFilter = (queryString) => {
  return (supply) => {
    return supply.supplyName.indexOf(queryString) != -1;
  }
}
