<template>
    ...
    <el-table :data="placePageList" style="width: 100%; margin-top: 10px;" table-layout="auto" size="large" border stripe>
      ...
      <el-table-column prop="introduce" label="产地介绍" sortable/>
      <el-table-column label="操作">
        <template #default="props">
          <el-button type="primary" title="修改产地" :icon="Edit" circle @click="openPlaceUpdate(props.row)" />
          <el-button type="danger" title="删除产地" :icon="Delete" circle @click="deletePlace(props.row.placeId)" />
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页 -->
  ...
  const export2Table = () => {
    get("/place/exportTable", params).then(result => {
      ...
      const columns = [
        ...
        {"title": "产地介绍", "key": "introduce"},
      ];
      export2excel(columns, storeList, "产地信息表");
    });
  }
